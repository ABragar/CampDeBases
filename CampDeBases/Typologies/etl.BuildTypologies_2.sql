USE [AmauryVUC]
GO
/****** Object:  StoredProcedure [etl].[BuildTypologies_2]    Script Date: 11/03/2015 11:21:06 ******/
SET ANSI_NULLS ON
GO
...
ALTER proc [etl].[BuildTypologies_2]
as
...
-- Modified by :		Philippe TOULEMONT
-- Modifications :	use of the table #T_Deleted_Typos to perform all deletion
-- Modification date :	09/11/2015
-- Modified by :		Andrei BRAGAR
-- Modifications : add web activity to PM*, PG*, OE* typologies  
-- =============================================
...
, t.MarqueID
, a.ConsentementDate
FROM #T_contacts t JOIN dbo.ConsentementsEmail a on t.ConsentementID = a.ConsentementID
WHERE  (a.DerniereOuvertureDate>=DATEADD(month,-6,getdate()) or a.DernierClickDate>=DATEADD(month,-6,getdate()))
...
FROM   
#T_contacts t JOIN etl.VisitesWeb AS vw ON t.MasterID = vw.MasterID  
       INNER JOIN ref.SitesWeb  AS sw
            ON  vw.SiteId = sw.WebSiteID AND sw.Marque = t.MarqueID
GROUP BY
       vw.MasterID
      ,marque
...
SELECT a.MasterID
      ...
      ,a.ConsentementDate
FROM   #T_contacts t
       JOIN dbo.ConsentementsEmail a
            ON  t.ConsentementID = a.ConsentementID
WHERE  (
           (
           ...
           AND (
                   a.DernierClickDate < DATEADD(MONTH ,-6 ,GETDATE())
...
SELECT vw.masterID
      ...
      ,MAX(vw.DateVisite)          maxDateVisite
FROM   #T_contacts t
       JOIN etl.VisitesWeb      AS vw
            ON  t.MasterID = vw.MasterID
       INNER JOIN ref.SitesWeb  AS sw
            ON  vw.SiteId = sw.WebSiteID AND sw.Marque = t.MarqueID
GROUP BY
       vw.MasterID
...
FROM   #T_contacts t
       JOIN dbo.ConsentementsEmail a
            ON  t.ConsentementID = a.ConsentementID
WHERE  a.DerniereOuvertureDate >= DATEADD(MONTH ,-6 ,GETDATE())
...
WHERE  (
           ...
           OR (
                  a.DerniereOuvertureDate IS NULL
                  AND a.DernierClickDate IS NULL
              )
       )
--web activity
INSERT #T_Abos
...
FROM   #T_contacts t
       ...
       INNER JOIN ref.SitesWeb  AS sw
            ON  vw.SiteId = sw.WebSiteID
                AND sw.Marque = t.MarqueID
WHERE  vw.masterId IS NOT NULL
GROUP BY
       vw.MasterID
      ,marque
HAVING MAX(vw.DateVisite) >= DATEADD(MONTH ,-6 ,GETDATE())
...
create table #T_Deleted_Typos (TypoID int)


insert into #T_Deleted_Typos select a.TypoID -- delete a
...
and b.TypologieID = 69 -- Actuels
	...
	insert into #T_Deleted_Typos select b.TypoID -- delete b
	FROM   #T_Lignes_Typologies a
	       INNER JOIN #T_Lignes_Typologies b
	            ON  a.MasterID = b.MasterID
	                AND a.MarqueID = b.MarqueID
...
insert into #T_Deleted_Typos select t2.TypoID -- delete t2
...
, R.Appartenance 
from
#T_Lignes_Typologies T
inner join ref.Misc R on T.MarqueID = R.CodeValN and R.TypeRef = 'MARQUE'
where TypoID not in (select TypoId from #T_Deleted_Typos)
