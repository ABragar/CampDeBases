USE [AmauryVUC]
GO
/****** Object:  StoredProcedure [import].[PublierSDVP_Contrats]    Script Date: 07.05.2015 15:57:25 ******/
SET ANSI_NULLS ON
GO
...
-- a partir des fichiers SDVP_Contrats 
...
-- Publication des donnees de la table SDVP_Contrats
...
, null as MontantAbo -- sera recupere de RefTarifs
, N'EUR' as Devise
...
, a.CTRFID Fidelite
, null as ModeExpedition -- sera recupere de ModeExpedition
, 0 as SuspensionAbo -- sera recupere des suspensions en cours
...
, null as ModePaiement -- sera recupere de RefOffOptTit
, null as ValiditeCB -- sera recupere de RefCBValidites
, a.CTRNUMGCP
, a.CTRNUMFIN
, null as DateFinParutions -- sera calculee selon la regle a partir de SDVP_RefParutions
...
-- Retirer les "Erreurs operatrice"
delete a
...
set ProvenanceID=b.ProvenanceID
from #T_Lignes_SDVP_Contrats a inner join ref.Provenances b
on b.CodeProvenance=N'AUT' -- a ceux qui n'ont pas de correspondance, on attribue le code "AUTRES"
...
-- On va donc dedoublonner des tarifs et corriger les lignes affectees
...
-- Mettre a jour le montant dans les lignes affectees

create index idx_01_RefTarifs_Uniques on #T_SDVP_RefTarifs_Uniques(TARCODSOC,TARCODTIT,TARCODTAR)

-- Modif PHTO sur les remises
-- Modif AVE : application de la regle sur le ratio CTRNBRSOU / TARDUR
...
from #T_Lignes_SDVP_Contrats a 
inner join 
(select RANK() over (partition by CTRNUMABO,CTRCODSOC order by cast(ADRNUMINF as int) asc,newid()) as N1 -- on prend le plus recent : ADRNUMINF va en descendant, 999 etant le plus ancien
, CTRNUMABO
, CTRCODSOC
, ProfilID
...
and a.CTRNUMCTR=b.CTRNUMCTR
and b.SourceID=@SourceID -- SDVP
and a.ActionID=1 -- Modification des lignes ActionID=1 si deja inserees
...
-- prendre la derniere modification s'il y en a plusieurs
inner join (
...
-- On n'a plus besoin de #T_Lignes_SDVP_Contrats, le reste se fait a partir de brut.Contrats_Abos

if OBJECT_ID('tempdb..#T_Lignes_SDVP_Contrats') is not null
	drop table #T_Lignes_SDVP_Contrats

-- Maintenant, il faudra fabriquer les lignes dbo.Abonnements a partir des lignes de brut.Contrats_Abos
-- On cree une table temporaire avec les lignes nouvellement inserees dans brut.Contrats_Abos
...
and SourceID=@SourceID -- Donnees SDVP
...
, NOrder int NULL
, CTROPTOFF2 nvarchar(8) not null
)
...
-- #T_Contrat_MinMax - table temporaire des agregations
-- Elle servira de jointure avec la premiere et la derniere lignes de contrat de chaque abonnement
-- et fournira les sommes des montants et de remises

-- Puisque j'ai deja MasterAboID dans #T_Abo_Fusion_Coupure, je pourrais regrouper directement par MasterAboID

update a
set MasterAboID=b.MasterAboID,  CTROPTOFF = b.CTROPTOFF2
...
-- Table temporaire agregee ou il n'y aura qu'une ligne par abonnement
-- Apres divers enrichissements, elle alimentera la table dbo.Abonnements
...
(
ContratID int null
, MasterID int null -- sera recupere le cas echeant de dbo.Abonnement 
, MasterAboID int null
, ProfilID int null -- Champ de regroupement
, CTRCODSOC nvarchar(8) null -- Champ de regroupement
, CTRCODTIT nvarchar(8) null -- Champ de regroupement
...
and a.CTRCODPRV=b.CTRCODPRV
and a.CTROPTOFF=b.CTROPTOFF
and a.CTRNUMCTR=b.CTRNUMCTR_MAX -- On prend la ligne la plus recente car il y a plus a recuperer
and a.MasterAboID=b.MasterAboID  -- выбираем по b.CTRNUMCTR_MAX  

-- CatalogueAbosID est pris sur la derniere ligne, plus recente
-- Meme avec le regroupement par 6 champs, cette regle reste applicable

-- Ici, eliminer les doublons au niveau de MasterAboID 
...
-- Maintenant, recuperer ce qui nous est necessaire de la 1ere ligne, par jointure avec #T_Contrat_MinMax : 
...
-- Prendre la SouscriptionAboDate la plus ancienne renseignee
...
-- Prendre la ReaboDate la plus recente renseignee
update a
...
-- Prendre le ModeExpedition le plus recent renseigne
update a
...
-- A present, on peut alimenter la table etl.Abos_Agreg_SDVP
...
-- Son contenu sera deverse dans dbo.Abonnements 
-- par la procedure etl.InsertAbonnements_Agreg dans le cadre du process commun
...
-- Contrats BOX (Mode d'expedition : ABONNEMENT DIFFUSEUR)
